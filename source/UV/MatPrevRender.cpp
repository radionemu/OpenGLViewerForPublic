...
namespace UV
{
	...
	void MaterialPreviewRenderer::render(GLVGLTF::GLVMaterial *pMaterial)
	{
		...
		{
			predraw(mInspectorPixelSize.first, mInspectorPixelSize.second);
		...
		// onMouseDown(0, io.MousePos.x, io.MousePos.y);
	...
	void MaterialPreviewRenderer::ptov(int mouseX, int mouseY, float screenW, float screenH, glm::vec3 &vec)
	{
		...
		float id = (vec[0] * vec[0] + vec[1] * vec[1]);

		vec[2] = d > (r / glm::sqrt(2)) ? glm::sqrt(d) : ((r * r) / (2 * (glm::sqrt(id)))); // calculate z
		glm::normalize(vec);
	}
